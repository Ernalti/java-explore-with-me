...
public interface StatRepository extends JpaRepository<Stat, Long> {
	...
	@Query("SELECT NEW ru.practicum.statdto.StatResponceDto(s.app, s.uri, COUNT(DISTINCT s.ip))" +
			"FROM Stat s " +
	...
	@Query("SELECT NEW ru.practicum.statdto.StatResponceDto(s.app, s.uri, COUNT(s.ip)) " +
			"FROM Stat s " +
			"WHERE s.created BETWEEN :start AND :end " +
			"AND s.uri IN :uris " +
			"GROUP BY s.app, s.uri " +
